...
export default {
  convert: ({ form, submit, data }, dataTypes) => {
    ...
    if (!isEmpty(get(data, 'D1InternaltionExchangeHistoryFieldList', [])) && isEmpty(get(data, 'D2YouthInternshipExchangeHistoryFieldList', []))) {
      ...
      const D1InternaltionExchangeHistoryFieldListnod2 = get(data, 'D1InternaltionExchangeHistoryFieldList', []);

      if (!isEmpty(D1InternaltionExchangeHistoryFieldListnod2)) {
        const templist = D1InternaltionExchangeHistoryFieldListnod2;

        if (!isEmpty(templist[0].D1i_PreviousExchangeItem)) {
          mappingd1['201718A'] = (templist[0].D1i_PreviousExchangeItem.includes('201718') ? 'Yes' : '');
          mappingd1['201920A'] = (templist[0].D1i_PreviousExchangeItem.includes('201920') ? 'Yes' : '');
          mappingd1['202324A'] = (templist[0].D1i_PreviousExchangeItem.includes('202324') ? 'Yes' : '');
        }
        if (!isEmpty(templist[1].D1i_PreviousExchangeItem)) {
          mappingd1['201718B'] = (templist[1].D1i_PreviousExchangeItem.includes('201718') ? 'Yes' : '');
          mappingd1['201920B'] = (templist[1].D1i_PreviousExchangeItem.includes('201920') ? 'Yes' : '');
          mappingd1['202324B'] = (templist[1].D1i_PreviousExchangeItem.includes('202324') ? 'Yes' : '');
        }
        if (!isEmpty(templist[2].D1i_PreviousExchangeItem)) {
          mappingd1['201718C'] = (templist[2].D1i_PreviousExchangeItem.includes('201718') ? 'Yes' : '');
          mappingd1['201920C'] = (templist[2].D1i_PreviousExchangeItem.includes('201920') ? 'Yes' : '');
          mappingd1['202324C'] = (templist[2].D1i_PreviousExchangeItem.includes('202324') ? 'Yes' : '');
        }
      }
    ...
    if (!isEmpty(get(data, 'D2YouthInternshipExchangeHistoryFieldList', [])) && !isEmpty(get(data, 'D1InternaltionExchangeHistoryFieldList', []))) {
      ...
      const D1InternaltionExchangeHistoryFieldList = get(data, 'D1InternaltionExchangeHistoryFieldList', []);
      if (!isEmpty(D1InternaltionExchangeHistoryFieldList)) {
        const templist = D1InternaltionExchangeHistoryFieldList;
    ...
    data.organiserNameThree_fundingScheme = convertPartBCheckbox2str(get(data, 'organiserNameTwo_fundingScheme', []), i_hk);
