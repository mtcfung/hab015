...
export default {
  convert: ({ form, submit, data }, dataTypes) => {
    ...
    const i18n = i18nByLocale(form.locale);
    const convertPartBCheckbox2str = (list1, i18n) => {
      let result = '';
      ...
      if (!isEmpty(list1))
 {
        for (let i = 0; i < list1.length; i++) {
          if (isFirst) {
            ...
            result = `${i18n.translate(`field.label.partB.${list1[i]}`)}`;
          } else {
            result += ',' + `${i18n.translate(`field.label.partB.${list1[i]}`)}`;
          }
        }
    ...
    data.fundingScheme = convertPartBCheckbox2str(get(data, 'fundingScheme', []), i18n);
    data.theme = convertPartBCheckbox2str(get(data, 'theme', []), i18n);
    data.Q19vSelectMultiple = convertPartBCheckbox2str(get(data, 'Q19vSelectMultiple', []), i18n);
